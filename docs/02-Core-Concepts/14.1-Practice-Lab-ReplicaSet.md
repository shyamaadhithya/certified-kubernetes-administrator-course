...
🧩 **Key Functions (Self-Healing & Scaling):**
...
🧩 **Create ReplicaSet - IMPERATIVE** _(rare, but CKA asks sometimes)_
...
3️⃣ Apply the updated ReplicaSet
```bash
...
🔸 Useful Imperative Editing Commands
...
**Master Level Example – Scale all RS by label**
...
**FULL MASTER LAB (Copy–Paste Practice)**
...
> [!IMPORTANT]
>    - `selector.matchLabels` MUST match `template.labels`. If they don’t match → `ReplicaSet` will create 0 pods.
...
🧩 **Inspect Pods Controlled by a ReplicaSet**
...
🧩 **Inspect ReplicaSet YAML (Full Manifest)**
```bash
...
### 14.1.3 Delete Pod → Observe Re-creation
...
### 14.1.8 Convert ReplicaSet → Deployment
...
> [!WARNING] If a ReplicaSet is created and managed by a Deployment, you should NEVER manually edit, scale, or manage that ReplicaSet directly. You must manage only the Deployment.
