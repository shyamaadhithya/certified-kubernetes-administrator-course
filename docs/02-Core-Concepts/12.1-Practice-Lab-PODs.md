### 2.2.1 Create Pod (`Imperative` & `Declarative`)
...
```
kubectl run is only for Pods and basic Deployments — useful for quick tests.
...
### 2.2.2 Run Pod in Specific Namespace
...
### 2.2.3 Define Restart Policies (Always, OnFailure, Never)
In Kubernetes, restart policies define how a Pod’s containers should be restarted when they exit.
...
### 2.2.4 View Pod Logs (`logs`, `-f`, `--previous`, `--since`, `--since-time`)
...
### 2.2.5 Execute Commands Inside Pod (direct & via Bastion)
...
3️⃣ Now you can run `exec` commands normally on bastion server:
```bash
...
### 2.2.6 Environment Variables (env, envFrom, ConfigMap, Secret)

🧩 **`env` Define Environment Variables Directly in a Pod**
...
🧩 **Use `envFrom` to Import All Variables from a `ConfigMap`**
...
🧩 **Use `envFrom` to Import All Variables from a `Secret`**
...
!!! tip
    Values from secrets appear decoded inside containers — base64 decoding is only for manifest inspection.

🧩 **Use `env` to Reference Individual Keys from `ConfigMap` or `Secret`**
...
!!! Note
    ...
    # Describe Pod (shows source refs) Look under Environment: section — shows where each var came from (ConfigMap/Secret)
...
!!! info
    If `kubectl cp` fails (e.g. `tar` not found in container), Some minimal container images don’t have the `tar` binary, which `kubectl cp` uses internally.
...
### 2.2.8 Multi-container Pod (Sidecar)

In Kubernetes, a Pod is the smallest deployable unit — it can contain one or more containers that share the same:
...
A Sidecar container is a secondary container that runs alongside the main application container to enhance or support its functionality — without changing the main application code. Think of a sidecar motorcycle — the main bike (primary app) does the driving, and the sidecar helps carry gear or perform extra functions.
...
**Key Points:**
 ...
 - After all init containers finish, they’re **terminated**, and **app containers** start.
...
Here:
 ...
 - Once that’s done, it exits successfully.
 - The main container **(nginx)** starts running.

---

### 2.2.10 Ephemeral Containers _(for debugging)_

**Ephemeral Containers** are **temporary containers** you can **add to a running Pod for debugging purposes** — _without restarting or changing the original Pod spec._

**They’re useful because:**

 - Sometimes you can’t kubectl exec into a container (for example, if it has no shell, or is crash-looping).
...
**The ephemeral container:**
 ...
 - Does not appear in `.spec.containers` (it’s listed under `.spec.ephemeralContainers`).
...
2️⃣ Add an ephemeral container
```bash
...
_Explanation:_
 ...
 - `--target=webapp`: targets the main container’s namespaces
 - `--name=debugger`: gives a name to your ephemeral container
 - `-it`: interactive terminal

3️⃣ Inside the ephemeral container
...
5️⃣ Verify ephemeral container attached
...
!!! info
    Ephemeral containers can only be added via `kubectl debug` or **API**, but for understanding, here’s what it looks like inside the Pod manifest:
    ```yaml
    spec:
      ephemeralContainers:
      - name: debugger
        ...
        targetContainerName: webapp
    ```
    **Cleanup:** Ephemeral containers disappear when the Pod is deleted _— no cleanup needed_.


---

### 2.2.11 Pod PriorityClass
...
1️⃣ Create a PriorityClass _(Imperative style doesn’t exist, so YAML only)_
...
2️⃣ Create a Pod using this PriorityClass
...
3️⃣ Verify Priority of the Pod
...
4️⃣ Create a Low PriorityClass (to simulate eviction behavior)
...
5️⃣ Test Eviction Logic (Optional but great practice)
...
!!! note
    * PriorityClass is NOT namespaced It’s cluster-wide.
...
### 2.2.12 Manual Static Scheduling (via `nodeName`)
...
Kubelet on that node directly creates the Pod — ONLY if:

 * the node exists
 * the kubelet on that node sees the Pod definition
 * resources are available

If the `nodeName` is wrong or **unreachable** → Pod stays in **Pending** forever.

1️⃣ Identify Node Names:
...
2️⃣ Create a Pod Manifest with `nodeName`:
...
3️⃣ Verify the `Pod` is Running on the Correct `Node`:
...
🆚 `Manual Static Scheduling` vs `NodeSelector`

| Feature                     | nodeName          | nodeSelector                      |
| --------------------------- | ----------------- | --------------------------------- |
| Schedules without scheduler | ✅                 | ❌                               |
...
### 2.2.13 Delete Pod (effect on Deployment)
...
2️⃣ **Rolling Updates or Rollbacks:**
...
3️⃣ **Impact on Application Availability:**
...
4️⃣ **No Manual Intervention Needed:**
...
### 2.2.15 Debug Failing Pod (`kubectl debug`)
...
3️⃣ **Debugging with kubectl debug**
...
🧩 **Get the Pod IP**: A pod named `web-pod` is running in the `default` namespace.
```bash
...
🧩 **List all Pod IPs in a namespace**: In the namespace `dev`, list **all pod names and their IPs**.
```bash
kubectl get pods -n dev -o jsonpath='{range .items[*]}{.metadata.name}{" => "}{.status.podIP}{"\n"}{end}'
```

🧩 **Verify that a Pod has no IP yet**: A pod named `pending-app` is stuck in **Pending**. Check whether it has been assigned an IP.
```bash
kubectl get pod pending-app -o jsonpath='{.status.podIP}'
```
If it's empty, the pod has **no IP assigned**.

🧩 **Check Pod IP from inside the Pod**: Exec into pod `tools` and show its own IP from inside the container.
```bash
...
🧩 **Find Pod IP from a Service**: A Service named `backend-svc` selects Pods with label `app=backend`. List the IPs of Pods behind this Service.
```bash
kubectl get pods -l app=backend -o jsonpath='{range .items[*]}{.metadata.name}{" => "}{.status.podIP}{"\n"}{end}'
```
---

### 2.2.17 Get YAML of Existing Pod (kubectl get pod -o yaml)

🧩  **Lab-1 Export YAML and Recreate Pod**
...
🧩  **Lab-2 Create New Pod From Existing Pod’s YAML**
...
🧩  **Lab-3 Fix Immutable Field Error**
...
🧩  **Lab-4 Get a Clean YAML Without Status Fields**
...
🧩  **Lab-5 Extract Only the Pod Spec** _(useful for `replicasets`, `deployments`)_
...
🧩  **Lab-5 Generate YAML Template for a Pod** _(without existing pod)_
...
🧩  **Lab-6 Patch an Existing Pod** _(in-line)_
...
### 2.2.18 Use `kubectl explain` for field reference
...
To explore the `spec` field of a Pod resource, which contains information about the Pod’s desired state (such as containers, volumes, etc.), you would use:
